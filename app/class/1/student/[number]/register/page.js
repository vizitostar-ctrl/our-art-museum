...
export default function RegisterPage() {
  ...
  async function uploadArtworkImage(file, imageType) {
    const extension = getFileExtension(file);

    const filePath =
      `class-1/student-${studentNumber}/` +
  ...
  async function handleSubmit(event) {
    ...
    if (!originalFile || !parodyFile || !aiFile) {
      alert(
    ...
    try {
      ...
      const originalUrl = await uploadArtworkImage(
        originalFile,
        "original"
      );

      const parodyUrl = await uploadArtworkImage(
        parodyFile,
        "parody"
      );

      const aiUrl = await uploadArtworkImage(
        aiFile,
        "ai"
      );
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          display_name: `2-1 ${studentNumber}번`,
          title: `2학년 1반 ${studentNumber}번 작품`,
          original_url: originalUrl,
          parody_url: parodyUrl,
          ai_url: aiUrl,
          feature: originalFeature.trim(),
          intent: expressionIntent.trim(),
          prompt_text: aiFeature.trim(),
          status: "pending",
        });
